...
public class ExampleStepManager implements IStepManager {
	...
	public void add(BeanPlan plan, String name, String planstartdate,
		...
		if(name == null || "".equals(name))throw new BusinessException("计划步骤名不能为空");
		...
		try {
			...
			if(rs.next()) {
				rs.close();
				pst.close();
				throw new BusinessException("该计划步骤名已存在");
			}
			rs.close();
			pst.close();
			...
			sql = "insert into tbl_step(plan_id,step_order,step_name,"
	...
	public void deleteStep(BeanStep step) throws BaseException {
		...
		try {
			...
			if(!BeanUser.currentLoginUser.getUser_id().equals(plan_user_id)) {
				st.close();
				throw new BusinessException("不能删除其他用户的步骤");
			}
			
			String sql4 = "select real_begin_time,real_end_time from tbl_step where step_id = " + step_id;
			java.sql.Statement st4 = conn.createStatement();
			java.sql.ResultSet rs4 = st.executeQuery(sql4);
			String real_begin_time = null;
			String real_end_time = null;
			while(rs4.next()) {
				real_begin_time = rs4.getString(1);
				real_end_time = rs4.getString(2);
			}
			st4.close();
			rs4.close();
			...
			if((real_end_time == null) && flag == 0) {		//若结束时间为空，开始时间不为空，则已开始步骤数-1
				if(real_begin_time != null) {
					flag = 1;
				}
			}
			if((real_end_time != null) && flag == 0) { 	//若结束时间不为空，则已结束步骤数-1
				flag = 2;
			}
			
			if(flag == 1) {
				String sql2 = "update tbl_plan set start_step_count = start_step_count - 1 where plan_id = ?";
				java.sql.PreparedStatement pst2 = conn.prepareStatement(sql2);
				pst2.setInt(1, plan_id);
				pst2.execute();
				pst2.close();
			}
			
			if(flag == 2) {
				String sql3 = "update tbl_plan set finished_step_count = finished_step_count - 1 where plan_id = ?";
				java.sql.PreparedStatement pst3 = conn.prepareStatement(sql3);
				pst3.setInt(1, plan_id);
				pst3.execute();
				pst3.close();
			}
